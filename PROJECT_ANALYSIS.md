# تحليل شامل لمشروع FTD TechZone (متجر الفجر للتجارة الإلكتروني) - بنية Clean Architecture

مرحباً بك! يهدف هذا الملف إلى تقديم تحليل هيكلي وتقني مفصل ومكامل لمشروع **FTD TechZone**، وهو عبارة عن منصة تجارة إلكترونية متكاملة مصممة خصيصاً لشركة **الفجر للتجارة (Alfajr For Trade)** في مصر. تم بناء المشروع بالاعتماد على نمط **البنية النظيفة (Clean Architecture)** مقسماً إلى 4 مشاريع فرعية باستخدام بيئة عمل **ASP.NET Core 9.0 MVC** مع نظام **Entity Framework Core 9.0** وقاعدة بيانات **SQL Server**.

---

## 📌 جدول المحتويات
1. [نظرة عامة على المشروع والتقنيات المستخدمة](#1-نظرة-عامة-على-المشروع-والتقنيات-المستخدمة)
2. [هيكل المجلدات وتقسيم المشاريع (Clean Architecture Structure)](#2-هيكل-المجلدات-وتقسيم-المشاريع-clean-architecture-structure)
3. [تحليل قاعدة البيانات ونماذج البيانات (Domain Entities)](#3-تحليل-قاعدة-البيانات-ونماذج-البيانات-domain-entities)
4. [بذر البيانات الافتراضية (Database Seeding)](#4-بذر-البيانات-الافتراضية-database-seeding)
5. [تحليل طبقة التطبيق والخدمات (Application Layer & Services)](#5-تحليل-طبقة-التطبيق-والخدمات-application-layer--services)
6. [تحليل طبقة التحكم والمتحكمات (Controllers Layer)](#6-تحليل-طبقة-التحكم-والمتحكمات-controllers-layer)
7. [الواجهات والتنسيق البصري (Views & Layouts)](#7-الواجهات-والتنسيق-البصري-views--layouts)
8. [دليل التشغيل وخطوات البدء في التطوير](#8-دليل-التشغيل-وخطوات-البدء-في-التطوير)
9. [أفكار مقترحة للتطوير المستقبلي](#9-أفكار-مقترحة-للتطوير-المستقبلي)
...
* **إطار العمل:** ASP.NET Core 9.0 MVC (بنية MVC نقية ومخصصة بالكامل بدون استخدام Razor Pages لضمان سرعة الاستجابة والمرونة التامة).
* **قاعدة البيانات:** SQL Server مع EF Core 9.0 لربط وإدارة الكائنات البرمجية بالجداول (Code-First Approach).
...
## 2. هيكل المجلدات وتقسيم المشاريع (Clean Architecture Structure)

يتكون الحل (Solution) في مسار [FTD.Web.sln](file:///c:/Users/dell/Documents/unigroup/New%20folder/FTD.Web/FTD.Web.sln) من أربعة مشاريع رئيسية لضمان الفصل التام للمسؤوليات (Separation of Concerns):

```mermaid
graph TD
    Web[FTD.Web - Presentation] --> Application[FTD.Application - Logic]
    Web --> Infrastructure[FTD.Infrastructure - Data/External]
    Infrastructure --> Application
    Application --> Domain[FTD.Domain - Core]
```

### 1. طبقة النطاق: [FTD.Domain](file:///c:/Users/dell/Documents/unigroup/New%20folder/FTD.Domain)
تمثل قلب النظام وهي مستقلة تماماً ولا تعتمد على أي طبقة أخرى:
* 📂 **[Entities](file:///c:/Users/dell/Documents/unigroup/New%20folder/FTD.Domain/Entities):** تضم جميع نماذج البيانات الأساسية للمشروع (مثل `Product.cs`, `Category.cs`, `Brand.cs`).

### 2. طبقة التطبيق: [FTD.Application](file:///c:/Users/dell/Documents/unigroup/New%20folder/FTD.Application)
تحتوي على منطق الأعمال وواجهات الاستخدام ولا تعتمد على تفاصيل البنية التحتية:
* 📂 **[Interfaces](file:///c:/Users/dell/Documents/unigroup/New%20folder/FTD.Application/Interfaces):** تضم واجهات التخاطب والخدمات (مثل `IAppDbContext.cs`, `IProductService.cs`).
* 📂 **[Services](file:///c:/Users/dell/Documents/unigroup/New%20folder/FTD.Application/Services):** تضم التنفيذ الفعلي لخدمات الأعمال (مثل `ProductService.cs`, `OrderService.cs`).
* 📂 **[DTOs](file:///c:/Users/dell/Documents/unigroup/New%20folder/FTD.Application/DTOs):** كائنات نقل البيانات المعزولة عن موديلات قاعدة البيانات لمنع تسريب التفاصيل للواجهات.
* 📂 **[Mappers](file:///c:/Users/dell/Documents/unigroup/New%20folder/FTD.Application/Mappers):** تحتوي على دوال التحويل التلقائي بين الموديلات والـ DTOs.

### 3. طبقة البنية التحتية: [FTD.Infrastructure](file:///c:/Users/dell/Documents/unigroup/New%20folder/FTD.Infrastructure)
تضم التفاصيل التقنية الخارجية والاتصال بقاعدة البيانات:
* 📂 **[Data](file:///c:/Users/dell/Documents/unigroup/New%20folder/FTD.Infrastructure/Data):** تضم سياق قاعدة البيانات [AppDbContext.cs](file:///c:/Users/dell/Documents/unigroup/New%20folder/FTD.Infrastructure/Data/AppDbContext.cs) الذي يربط الكائنات البرمجية بـ SQL Server.
* 📂 **[Migrations](file:///c:/Users/dell/Documents/unigroup/New%20folder/FTD.Infrastructure/Migrations):** ملفات تتبع وتحديث هيكلية الجداول.
* 📂 **[Services](file:///c:/Users/dell/Documents/unigroup/New%20folder/FTD.Infrastructure/Services):** تضم تطبيقات الخدمات التقنية مثل البريد الإلكتروني [EmailService.cs](file:///c:/Users/dell/Documents/unigroup/New%20folder/FTD.Infrastructure/Services/EmailService.cs).

### 4. طبقة العرض: [FTD.Web](file:///c:/Users/dell/Documents/unigroup/New%20folder/FTD.Web)
تمثل مدخل النظام للمستخدمين (ASP.NET Core MVC):
* 📂 **[Controllers](file:///c:/Users/dell/Documents/unigroup/New%20folder/FTD.Web/Controllers):** معالجة الطلبات القادمة وتوجيهها للمستخدم. يضم قسماً خاصاً بلوحة التحكم في المجلد الفرعي `Admin`.
* 📂 **[ViewModels](file:///c:/Users/dell/Documents/unigroup/New%20folder/FTD.Web/ViewModels):** موديلات العرض المخصصة لواجهات cshtml.
* 📂 **[Views](file:///c:/Users/dell/Documents/unigroup/New%20folder/FTD.Web/Views):** قوالب واجهة المستخدم بصيغة cshtml.
* 📂 **[wwwroot](file:///c:/Users/dell/Documents/unigroup/New%20folder/FTD.Web/wwwroot):** ملفات الاستايل والثيم (CSS, JS) المخصصة للموقع وصور المنتجات.
* 📄 **[Program.cs](file:///c:/Users/dell/Documents/unigroup/New%20folder/FTD.Web/Program.cs):** إعدادات حقن الاعتماديات (DI)، مسارات الـ MVC، وضبط الحماية والمصادقة وجلسة الخادم.

---

## 3. تحليل قاعدة البيانات ونماذج البيانات (Domain Entities)

تتألف قاعدة البيانات من مجموعة من الكيانات المترابطة المعرفة داخل المجلد [FTD.Domain/Entities](file:///c:/Users/dell/Documents/unigroup/New%20folder/FTD.Domain/Entities) لدعم ثنائية اللغة:
...
### تفاصيل الكائنات البرمجية:

1. **Brand (العلامات التجارية):**
   * تمثل الشركات المصنعة للأجهزة المعروضة (مثل DOOGEE, Dreame, JisuLife).
   * تحتوي على الاسم باللغتين العربية والانجليزية (`NameAr`, `NameEn`)، الاسم اللطيف للرابط (`Slug`)، الشعار (`LogoPath`)، صورة البانر (`BannerPath`)، ووصف تفصيلي.
...
في طبقة البنية التحتية [AppDbContext.cs](file:///c:/Users/dell/Documents/unigroup/New%20folder/FTD.Infrastructure/Data/AppDbContext.cs) والملف [Program.cs](file:///c:/Users/dell/Documents/unigroup/New%20folder/FTD.Web/Program.cs), يتم فحص وبذر البيانات التالية بشكل تلقائي فور تشغيل البرنامج لضمان عمل النظام فوراً دون إعدادات يدوية معقدة:

* **حساب المسؤول الافتراضي (Admin Identity User):**
  * **البريد الإلكتروني:** `admin@ftdtechzone.com`
  * **كلمة المرور:** `Admin@123456`
  * **الصلاحية:** يسند المشرف تلقائياً إلى دور الإدارة `Admin` ليتمكن من الوصول المباشر للوحة التحكم.
...
## 5. تحليل طبقة التطبيق والخدمات (Application Layer & Services)

تحتوي هذه الطبقة في مشروع [FTD.Application](file:///c:/Users/dell/Documents/unigroup/New%20folder/FTD.Application/Services) على منطق العمل المعزول لتسهيل قراءة الكود وإعادة استخدامه:
...
4. **CartService (خدمات سلة التسوق):**
   * تعتمد على مكتبة تحويل البيانات وتخزينها في جلسة الخادم (Session-Based JSON Serialization) وتطبق الواجهة `ICartService`.
   * تقوم بقراءة وتخزين كائن السلة المبسط `RawCartItem` (الذي يحتوي فقط على معرف المنتج والكمية) ثم تقوم ببناء السلة الكلية `CartDto` وجلب تفاصيل المنتجات والأسعار الحالية من قاعدة البيانات للتأكد من أن السلة تعرض دوماً الأسعار المحدثة.
5. **EmailService (خدمات البريد الإلكتروني - منفذة في Infrastructure):**
   * تعتمد على بروتوكول SMTP لإرسال رسائل بريدية منسقة بصيغة HTML لإخطار المسؤولين فور استقبال رسالة اتصال جديدة، مع آلية حماية تضمن عدم توقف أو تعطل النظام في حال فشل إرسال البريد أو عدم ضبط إعدادات الاتصال.

---

## 6. تحليل طبقة التحكم والمتحكمات (Controllers Layer)

تقع المتحكمات في مجلد [FTD.Web/Controllers](file:///c:/Users/dell/Documents/unigroup/New%20folder/FTD.Web/Controllers) وتعتبر المحرك التوجيهي للتطبيق:
...
* **OrderController:** يتحكم في صفحة تأكيد بيانات العميل وعنوانه لإجراء عملية الدفع (Checkout) والتحقق من صحة المدخلات وسحب عناصر السلة المسجلة في الـ Session، ثم التحويل لصفحة نجاح الطلب وإفراغ السلة.
...
* **AdminAccountController:** يسمح للمشرفين بتسجيل الدخول بأمان إلى لوحة الإدارة وتوليد كعكة التحقق (Identity Cookie).

---

## 7. الواجهات والتنسيق البصري (Views & Layouts)

يعتمد التنسيق في واجهات مشروع [FTD.Web](file:///c:/Users/dell/Documents/unigroup/New%20folder/FTD.Web) على تصميم خفيف الوزن، متجاوب، وداعم للغات المتعددة باستخدام **Vanilla CSS** مخصص بالكامل في مجلد [wwwroot/css](file:///c:/Users/dell/Documents/unigroup/New%20folder/FTD.Web/wwwroot/css):
...
## 8. دليل التشغيل وخطوات البدء في التطوير
...
1. افتح ملف [appsettings.json](file:///c:/Users/dell/Documents/unigroup/New%20folder/FTD.Web/appsettings.json) وتأكد من تحديث نص الاتصال بقاعدة البيانات `DefaultConnection` ليتوافق مع خادم SQL Server المحلي الخاص بك:
   ...
   ```
2. افتح سطر الأوامر في مسار المشروع وقم بتطبيق الهجرة لإنشاء جداول قاعدة البيانات وبذر البيانات الافتراضية:
   ```bash
...
```
* سيقوم المتصفح بفتح الموقع تلقائياً على الرابط المحدد في ملف الإعدادات.
* للوصول للوحة التحكم، توجه للمسار التالي وسجل الدخول بالحساب الافتراضي:
  * **المسار:** `https://localhost:7000/Admin` (أو البورت الفعلي لديك)
  * **البريد:** `admin@ftdtechzone.com`
  * **كلمة المرور:** `Admin@123456`

---

## 9. أفكار مقترحة للتطوير المستقبلي
