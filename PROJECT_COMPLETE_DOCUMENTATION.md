...
* **Category.cs**: يمثل الأقسام الرئيسية للمنتجات ويحتوي على الاسم ورابط الصورة والأيقونة التعبيرية (Emoji) والخاصية الجديدة للتحكم بالظهور في الصفحة الرئيسية (ShowOnHomepage).
...
* **IContentService.cs**: جلب وتحديث الإعدادات العامة للموقع والصفحات وقوائم التنقل (مع إمكانية حذف الصفحات).
...
## 7. تطوير لوحة تحكم محتوى الرئيسية الديناميكي (Dynamic Homepage Control Dashboard)

تم تحويل الصفحة الرئيسية إلى صفحة ديناميكية بالكامل تخضع بالكامل للتحكم من لوحة الإدارة (`Admin/Content/Blocks`) مقسمة إلى 5 تبويبات تفاعلية:

1. **البيانات والخصائص الجديدة:**
   * **إضافة خاصية `ShowOnHomepage` للكيان `Category`:** لتحديد إظهار التصنيف بالرئيسية مستقلة عن نشاطه العام.
   * **تعديل الـ Seed Data بقاعدة البيانات:** إضافة 43 كتل محتوى نصوص و14 إعداداً للنظام لتغطية كافة نصوص، أرقام، ترتيب، وإظهار أقسام الرئيسية.
   * **ترتيب الأقسام تفاعلياً:** خيار `homepage.sections.order` يحدد تسلسل أقسام الصفحة الرئيسية (Dispatcher) من خلال الإدارة.

2. **الواجهات المحدثة:**
   * **لوحة التحكم (`Blocks.cshtml`):** واجهة تبويبات متطورة بنظام التبويب والتفاعلية (Tabbed UI)، وقائمة منسدلة متعددة الاختيارات تفاعلية بالكامل لترتيب المنتجات بأسهم مخصصة، واختيار الأيقونات حياً من مكتبة Bootstrap Icons.
   * **رفع صور التصنيفات:** نموذج `Categories/Form.cshtml` يدعم رفع صور للمنتجات والتحقق من الحجم والامتداد وعرضها في الصفحة الرئيسية.
   * **الصفحة الرئيسية (`Index.cshtml`):** صياغة أقسام الرئيسية كدوال برمجية متزامنة (`async Task Render...`) لتفادي مشاكل الـ TagHelpers وتمرير العرض بشكل ديناميكي كامل.

## 8. تطوير منشئ الصفحات المرئي التفاعلي (Ultimate Drag-and-Drop Visual Page Builder)

تم تطوير نظام إدارة "الصفحات الحرة" بالكامل ليصبح بيئة عمل متكاملة لبناء الصفحات وتخصيصها بمرونة كاملة وسحبها وإفلاتها:

1. **الواجهات الموحدة والتفاعلية:**
   * **محرر موحد (`PageForm.cshtml`):** يجمع إعدادات الصفحة والسيو (SEO) في شريط جانبي مع شاشة عرض وسحب الأقسام (Canvas) بنظام الأكورديون. تم إزالة شاشة `PageSections` القديمة وعمل توجيه تلقائي لها.
   * **محرر نصوص Quill.js:** مدمج بالكامل باللغتين العربية والإنجليزية ويدعم التنسيق البصري ورفع الصور مباشرة إلى السيرفر وحفظ روابطها.
   * **السحب والإفلات (Drag & Drop):** مقبض سحب `bi-grip-vertical` لكل قسم يتيح الترتيب اللحظي وحفظه فورياً عبر AJAX للـ endpoint الجديد `UpdateSortOrders`.
   * **إمكانية حذف الصفحات:** إضافة زر الحذف للواجهة وربطه بـ `DeletePage` في الأدمن و `DeletePageAsync` في الخدمة البرمجية لحذف الصفحة وأقسامها كـ Cascade Delete تلقائياً.

2. **أقسام جديدة وخيارات مظهر متقدمة:**
   * **أقسام مبتكرة:** تم إضافة أقسام للـ Video (يوتيوب/فيميو/MP4) وقسم الـ Gallery (شبكة صور مع Lightbox) وقسم الـ Testimonials (آراء العملاء مع صور وتقييم بالنجوم).
   * **تخصيص المظهر:** خيار لتغيير لون الخلفية، الهوامش الداخلية (Padding)، ومحاذاة النصوص لكل قسم بشكل مستقل عبر `SectionStyleHelper` و `_SectionStyleControls.cshtml`.
   * **قالب عام موحد:** تُعرض كافة الصفحات الحرة تلقائياً داخل القالب الموحد للموقع (`_Layout.cshtml`) محتفظة باللوجو والهيدر والفوتر والـ Breadcrumbs.


## 9. تحسينات تجربة تصنيفات ومواصفات المنتجات (Categories & Attributes UX Innovations)

تم تحسين وتكامل التصنيفات والمواصفات بشكل تفاعلي وتجاري عميق لزيادة المبيعات وسرعة إدارة المنتجات:

1. **تكامل التصنيفات التفاعلي:**
   * **شريط التنقل (Navbar Hover Dropdown):** تحويل زر "المنتجات" بقائمة سطح المكتب إلى Hover Dropdown منبثق يعرض كافة التصنيفات النشطة ديناميكياً مع عدد المنتجات بكل تصنيف.
   * **أكورديون الموبايل:** إتاحة تصفح الكاتيجوريز بسهولة في الموبايل عبر أكورديون منسدل.
   * **تكامل الفوتر:** إدراج عمود التصنيفات النشطة تلقائياً بالفوتر.

2. **تسريع إدارة وإدخال المنتجات (Product Management UX):**
   * **تكرار المنتجات (Duplicate Product):** إضافة زر "نسخ" يتيح نسخ منتج كامل مع مواصفاته وصوره وبياناته كمسودة، لتسهيل إدراج المنتجات الشبيهة.
   * **إضافة قيم المواصفات اللحظية (Inline Quick Add):** إمكانية إضافة قيم مواصفات جديدة (مثل رامات أو ألوان جديدة) مباشرة عبر AJAX من داخل شاشة تعديل المنتج بالنقر على زر `+` بجانب حقل الاختيار.
   * **المواصفات الحرة (Free-Text Attributes):** إتاحة إدخال قيم نصية حرة للمواصفة التي لا تملك خيارات مسبقة.

3. **الفلترة الذكية وترتيب القيم:**
    * **الترتيب الطبيعي للمواصفات الرقمية:** ترتيب قيم المواصفات التقنية رقمياً (مثل 8GB < 12GB < 128GB) بدلاً من الترتيب الأبجدي الافتراضي لتسهيل الفلترة على الزوار.


## 10. ميزات المنتج الإضافية الديناميكية - محتويات العلبة (Dynamic Product Features)

تم تحويل قسم "ماذا يوجد في العلبة / حول المنتج" (4 مربعات) ليكون ديناميكياً وتفاعلياً بالكامل لكل منتج على حدة:

1. **التعديلات الهيكلية وقاعدة البيانات:**
   * **إضافة عمود `FeaturesJson` لجدول `Products`:** لتخزين مصفوفة الخصائص المخصصة للمنتج (التي تحتوي على اسم الأيقونة والنصوص باللغتين العربية والإنجليزية).
   * **إدراج EF Migration جديد:** إنشاء وتطبيق الهجرة `AddProductFeaturesJson` لترقية مخطط قاعدة البيانات محلياً.

2. **لوحة تحكم المنتج (Admin Panel UI):**
   * **كارت إدارة الميزات (`Form.cshtml`):** إضافة قسم تفاعلي جديد بالكامل يسمح للمسؤول بإضافة ميزات إضافية (محتويات العلبة) للمنتج، تحديد أيقوناتها (ضمان، شحن، إرجاع، تقسيط، إلخ) وكتابة قيمها وتعديلها أو حذفها لحظياً، مع حفظها كـ JSON.
